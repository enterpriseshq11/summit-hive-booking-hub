...
import azTotalFitnessLogo from "/assets/az-total-fitness-logo.png";
...
export function Header() {
  ...
  const renderNavItem = (item: typeof navItems[0]) => {
    ...
    const isFitness = item.href === "/fitness";
    
    const linkContent = isFitness ? (
      <Link
        key={item.href}
        to={item.href}
        className={cn(
          ...
          isActive
            ...
            : "text-primary-foreground/70 hover:text-accent hover:bg-accent/10"
        )}
      >
        <img 
          src={azTotalFitnessLogo} 
          alt="A-Z Total Fitness" 
