...
import {
  CreditCard, CheckCircle, Copy, TestTube, Unplug, Clock, Zap, Settings2, ArrowLeft,
} from "lucide-react";
    ...
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          <Button variant="outline" size="sm" className="border-zinc-600 text-zinc-200 hover:bg-zinc-700" onClick={() => navigate("/admin/settings/integrations")}>
            <ArrowLeft className="h-4 w-4 mr-1" /> Back to Integrations
          </Button>
        </div>
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center gap-2">
            <CreditCard className="h-6 w-6 text-green-400" /> Stripe Connection
          </h1>
          <p className="text-zinc-300">Manage your Stripe payment integration</p>
        </div>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="p-4 border border-zinc-700 rounded-lg bg-zinc-800/50">
                <p className="text-zinc-300 text-sm">Account Status</p>
                <div className="flex items-center gap-2 mt-1">
                  <Badge className="bg-green-500/20 text-green-400"><CheckCircle className="h-3 w-3 mr-1" />Connected</Badge>
                </div>
                <p className="text-zinc-300 text-xs mt-2">Stripe secret key is configured</p>
              </div>
              <div className="p-4 border border-zinc-700 rounded-lg bg-zinc-800/50">
                <p className="text-zinc-300 text-sm">Webhook Secret</p>
                <div className="flex items-center gap-2 mt-1">
                  <Badge className="bg-green-500/20 text-green-400"><CheckCircle className="h-3 w-3 mr-1" />Configured</Badge>
                </div>
                <p className="text-zinc-300 text-xs mt-2">STRIPE_WEBHOOK_SECRET is set</p>
              </div>
              <div className="p-4 border border-zinc-700 rounded-lg bg-zinc-800/50">
                <p className="text-zinc-300 text-sm">Last Webhook Received</p>
                <div className="flex items-center gap-2 mt-1">
                  {lastWebhookEvent ? (
                    <span className="text-white text-sm flex items-center gap-1">
                      <Clock className="h-3.5 w-3.5 text-zinc-400" />
                      {format(new Date(lastWebhookEvent.received_at), "MMM d, yyyy 'at' h:mm a")}
                      <Badge variant="outline" className="ml-2 text-xs border-zinc-600 text-zinc-200">{lastWebhookEvent.event_type}</Badge>
                    </span>
                  ) : (
                    <span className="text-zinc-300 text-sm">No webhooks received yet</span>
                  )}
                </div>
              </div>
              <div className="p-4 border border-zinc-700 rounded-lg bg-zinc-800/50">
                <p className="text-zinc-300 text-sm">Last Payment Synced</p>
                <div className="flex items-center gap-2 mt-1">
                  {lastTransaction ? (
                    <span className="text-white text-sm flex items-center gap-1">
                      <Clock className="h-3.5 w-3.5 text-zinc-400" />
                    ...
                    <span className="text-zinc-300 text-sm">No payments synced yet</span>
                  )}
                </div>
              </div>
            </div>
            <div className="flex gap-2">
              <Button variant="outline" className="border-zinc-600 text-zinc-200 hover:bg-zinc-700" onClick={handleTestConnection}>
                <TestTube className="h-4 w-4 mr-2" /> Test Connection
              </Button>
              <Button variant="outline" className="border-red-500/30 text-red-400 hover:bg-red-500/10" onClick={() => setShowDisconnect(true)}>
        ...
        <Card className="border-zinc-700 bg-zinc-900">
          ...
          <CardContent className="space-y-4">
            <div className="p-4 border border-zinc-700 rounded-lg bg-zinc-800/50">
              <Label className="text-zinc-300 text-sm">Webhook Endpoint URL</Label>
              <div className="flex items-center gap-2 mt-2">
                <code className="flex-1 p-2 bg-zinc-800 border border-zinc-600 rounded text-amber-400 font-mono text-sm truncate">
                  {webhookUrl}
                </code>
                <Button size="sm" variant="outline" className="border-zinc-600 text-zinc-200 hover:bg-zinc-700" onClick={() => copyToClipboard(webhookUrl)}>
            ...
            <div className="p-4 border border-zinc-700 rounded-lg bg-zinc-800/50">
              ...
              <ol className="text-zinc-300 text-sm space-y-1 list-decimal pl-5">
              ...
              <div className="flex flex-wrap gap-1.5 mt-2 ml-5">
                {[
                  ...
                  <Badge key={evt} variant="outline" className="text-xs font-mono border-zinc-600 text-zinc-200">{evt}</Badge>
                ))}
              </div>
            </div>
          </CardContent>
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between p-4 border border-zinc-700 rounded-lg bg-zinc-800/50">
              <div>
                ...
                <p className="text-zinc-300 text-xs">When a Stripe payment succeeds, automatically create a revenue event</p>
              </div>
              <Switch
            ...
            <div className="flex items-center justify-between p-4 border border-zinc-700 rounded-lg bg-zinc-800/50">
              <div>
                ...
                <p className="text-zinc-300 text-xs">When a Stripe-sourced revenue event is created, auto-calculate commissions</p>
              </div>
              <Switch
        ...
        <AlertDialog open={showDisconnect} onOpenChange={setShowDisconnect}>
          <AlertDialogContent className="bg-zinc-900 border-zinc-700">
            ...
            <AlertDialogFooter>
              <AlertDialogCancel className="border-zinc-600 text-zinc-200">Cancel</AlertDialogCancel>
